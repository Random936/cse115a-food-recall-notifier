...
with open(
    "food_recall_announcement_photo.json", "r", encoding="utf-8"
) as web_file:  # Image/Annoucement
...
def clean_company_name(name):
    name = re.sub(
        r"\b(Inc|LLC|Ltd|Co|Corporation|Company|Limited)\.?\b",
        "",
        name,
        flags=re.IGNORECASE,
    )
...
for web_recall in web_recalls:
    ...
    product_desc = clean_product_description(
        product_details.get("Product Description", "")
    )
    recall_reason = extract_recall_keywords(
        product_details.get("Reason for Announcement", "")
    )  # Match the info base on company name, product description and recall reason

    key = f"{company_name}|{product_desc}"
    web_dict[key] = {
        "url": web_recall.get("url"),
        "company_announcement": web_recall.get(
...
for api_recall in api_recalls:
    ...
    recall_reason_keywords = extract_recall_keywords(
        api_recall.get("reason_for_recall", "")
    )
    ...
    for web_key, web_data in web_dict.items():
        ...
        product_score = fuzz.partial_ratio(
            product_desc, web_product
        )  # product description
        common_keywords = (
            recall_reason_keywords & web_data["recall_reason_keywords"]
        )  # recall reason
        ...
        # Best match
        if (
            total_score > best_score and total_score >= 70
        ):  # Here I set as score as 70, you can modify this if you'd like to get higher accuracy
            best_match = web_data
            best_score = total_score

    # Merge the matched data
    if best_match:
        api_recall["company_announcement"] = best_match.get(
            "company_announcement", "No announcement found"
        )
